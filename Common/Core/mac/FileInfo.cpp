...
std::string FileInfo::getSymlinkTarget() const
{
	...
	if (!isCached())
	{
		if (exists())
		{
            ...
            int count = readlink(getCanonicalFilePath().c_str(), target, sizeof(target));
            if (count >= 0) {
                target[count] = '\0';
...
bool FileInfo::isDir() const
{
	...
	if (!isCached())
	{
		...
		if (stat(getCanonicalFilePath().c_str(), &localBuffer) == 0)
		{
			r = mCache.mIsDir = ((localBuffer.st_mode & S_IFDIR) > 0);
		}
	}
	return r;
}
...
bool FileInfo::isFile() const
{
	...
	if (!isCached())
	{
		...
		if (stat(getCanonicalFilePath().c_str(), &localBuffer) == 0)
		{
			r = mCache.mIsFile = ((localBuffer.st_mode & S_IFREG) > 0);
		}
	}
	return r;
}
...
bool FileInfo::isRoot() const
{
	...
	return isAbsolute() && path.size() > 0 &&
		(path[path.size() - 1] == '/' || path.find_last_of(kNativeDirSeparator) == 1);
}
