...
void PlatonicSolid::bakeCube()
{
	...
	mMesh.makeFace({ 0, 1, 2, 3 }); //0, 1, 2, 3
	mMesh.makeFace({ 4, 7, 6, 5 }); //4, 5, 6, 7
	mMesh.makeFace({ 5, 6, 2, 1 }); //8, 9, 10, 11  
	mMesh.makeFace({ 0, 3, 7, 4 }); //12, 13, 14, 15
	mMesh.makeFace({ 6, 7, 3, 2 }); //16, 17, 18, 19  
	mMesh.makeFace({ 4, 5, 1, 0 }); //20 21 22 23
...
void PlatonicSolid::bakeDodecahedron()
{
    ...
    //12 faces
	mMesh.makeFace({ 1, 9, 10, 5, 19 }); // 0, 1, 2, 3, 4
	mMesh.makeFace({ 1, 19, 16, 2, 12 }); //5 6 7 8 9
	mMesh.makeFace({ 0, 9, 1, 12, 13 }); //10 11 12 13 14
	mMesh.makeFace({ 0, 18, 4, 10, 9 }); //15 16 17 18 19
	mMesh.makeFace({ 4, 14, 15, 5, 10}); //20 21 22 23 24
	mMesh.makeFace({ 5, 15, 6, 16, 19}); //25 26, 27, 28, 29
	mMesh.makeFace({ 3, 8, 11, 7, 17 }); //30 31 32 33 34
	mMesh.makeFace({ 2, 8, 3, 13, 12 }); //35 36 37 38 39
	mMesh.makeFace({ 3, 17, 18, 0, 13}); //40 41 42 43 44
	mMesh.makeFace({ 4, 18, 17, 7, 14}); //15 46 47 48 49
	mMesh.makeFace({ 15, 14, 7, 11, 6}); //50 51 52 53 54
	mMesh.makeFace({ 2, 16, 6, 11, 8 }); //55 56 57 58 59 
...
void PlatonicSolid::bakeOctahedron()
{
	...
	mMesh.makeFace({ 0, 5, 3 }); // 0, 1, 2
	mMesh.makeFace({ 1, 3, 5 }); // 3, 4, 5
	mMesh.makeFace({ 1, 4, 3 }); // 6, 7, 8
	mMesh.makeFace({ 0, 3, 4 }); //9, 10, 11
	mMesh.makeFace({ 0, 2, 5 }); //12, 13, 14
	mMesh.makeFace({ 1, 5, 2 }); //15, 16, 17
	mMesh.makeFace({ 1, 2, 4 }); //18, 19, 20
	mMesh.makeFace({ 0, 4, 2 }); //21, 22, 23
...
void PlatonicSolid::bakeTetrahedron()
{
	...
	mMesh.makeFace({ 0, 1, 2 });
	mMesh.makeFace({ 1, 3, 2 });
	mMesh.makeFace({ 2, 3, 0 });
	mMesh.makeFace({ 0, 3, 1 });
