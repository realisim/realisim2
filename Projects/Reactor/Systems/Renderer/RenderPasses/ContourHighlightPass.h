...
namespace Reactor
{
    class ContourHighlightPass : public IRenderPass
    {
    public:
        ContourHighlightPass();
        ContourHighlightPass(const ContourHighlightPass&) = delete;
        ContourHighlightPass& operator=(const ContourHighlightPass&) = delete;
        virtual ~ContourHighlightPass();

    protected:
        virtual void initializeFbo() override final;
        virtual void loadShader(const std::string& iAssetPath) override final;
        //virtual void sharePasses(const std::map<int, IRenderPass*>& iRenderPassNameToRenderPass) final;
        ...
        void renderSelectedModels(const Rendering::Camera&, const std::vector<IRenderable*> iRenderables);
        void renderContour();
        ...
        Rendering::Shader mDrawSelectedModelsShader;
        Rendering::Shader mDrawContourShader;
        /*void applyBlur();
        void renderSelectedModels(const Rendering::Camera&, const std::vector<IRenderable*> iRenderables);
